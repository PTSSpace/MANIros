...
from maniros.msg import MoveControl
...
class VectorTranslation:
    ...
    """
    def __init__(self, length, width, scale):
        """
    ...
    def normalizeArray(self, array, threshhold = 1):
        ...
        """
        maxvalue = math.sqrt(2)  # abs(max(array, key=abs)) 
        if maxvalue > threshhold:
            for index, value in enumerate(array):
                array[index] = (value / maxvalue) * threshhold * self.trans_scale       # normalise and apply tranlational speed scaling factor
        return array
