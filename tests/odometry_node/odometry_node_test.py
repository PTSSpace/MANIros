...
class OdmTest(unittest.TestCase):
    ...
    def test_drive_odometry(self):
        ...
        self.assertItemsEqual(self.twist.angular, Vector3(v, 0, 0))
    ...
    def test_translation_f_odometry(self):
        ...
        while not rospy.is_shutdown() and not self.success and time.time() < timeout_t:
            ...
            rate.sleep()
        # Check for accurate odometry
        x = (r_enc + (p_enc/self.STEER_ENC_PPR))*2*math.pi*self.wheel_diameter
        self.assertItemsEqual(self.pose.position, Vector3(x, 0, 0))
    ...
    def test_tranlation_r_odometry(self):
        ...
        while not rospy.is_shutdown() and not self.success and time.time() < timeout_t:
            ...
            rate.sleep()
        # Check for accurate odometry
        y = (r_enc + (p_enc/self.STEER_ENC_PPR))*2*math.pi*self.wheel_diameter
        self.assertItemsEqual(self.pose.position, Vector3(0, y, 0))
